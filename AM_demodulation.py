...
# Keep the selected station bandwidth before extracting its amplitude envelope.
...
# Remove RF noise above the audible range before downsampling.
audio_filter = butter(
    5, AUDIO_CUTOFF, btype="lowpass", fs=SAMPLE_RATE, output="sos"
)
audio = sosfilt(audio_filter, amplitude)
...
ax2.set_ylabel("AM audio amplitude")
ax2.set_title("AM envelope audio vs time")
ax2.grid(True)
